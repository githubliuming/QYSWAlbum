...
class QYPhotoSever: NSObject {
    ...
    open func fetchAlbum(type: QYPhotoLibarayAssertType, result: @escaping (_: Array<QYGroup>) -> Void) {
        photoQueue.addOperation {
            ...
            for alubm: PHFetchResult in albums {
                alubm.enumerateObjects({ obj, _, _ in
    ...
    ///   - option: 可选参数
    /// - Returns: 返回遍历集合
    open func fetchCollection(collection: PHAssetCollection?, option _: PHFetchOptions?) -> Array<QYAsset> {
        ...
        photoQueue.addOperation {
            ...
            camerollAlbum.enumerateObjects { obj, _, _ in
                ...
                if obj.assetCollectionSubtype == .smartAlbumUserLibrary {
                    ...
                    let group: QYGroup = QYGroup(collection: obj, assetModels: assetModel)
                    groups.append(group)
                }
            }
            self.runOnMainThread {
    ...
    open func requestOriginImage(asset: QYAsset, completedHandler: QYPhotoSuccess?,
                                 progress: QYPhotoProgress? = nil) -> PHImageRequestID {
    ...
    open func requestThumbImage(asset: QYAsset,
                                size: CGSize,
                                completedHandler: QYPhotoSuccess?, progress: QYPhotoProgress? = nil) -> PHImageRequestID {
    ...
    open func requestVideo(_ asset: QYAsset, success: QYVideoSucces?, failure: QYPhotoError?, progress: QYPhotoProgress?) -> PHImageRequestID {
        ...
        videoOptions.progressHandler = { prog, error, _, _ in
            self.runOnMainThread {
                progress?(prog, error)
            }
        }
        ...
        photolibrary.performChanges({
            ...
            if let desc = desCollection {
                PHPhotoLibrary.shared().performChanges({
                    let changeRequest: PHAssetCollectionChangeRequest = PHAssetCollectionChangeRequest(for: desc)!
                    changeRequest.addAssets(asset as! NSFastEnumeration)
                }, completionHandler: { (succ: Bool, _: Error?) in
                    finishHandler(succ, asset: asset)
                })
            } else {
                finishHandler()
    ...
    open func deleMedailWithAsset(_ asset: QYAsset,
                                  ...
                                  completion: QYDeleteAssetCompletionBlock?) {
        func finishHanderl(success: Bool = false, error: Error? = nil) {
...
extension QYPhotoSever {
    ...
    ///   - fileUrl: 导出文件路径
    ///   - completion: 完成回调
    private func exportVideo(_ compostion: AVAsset, fileUrl: URL, completion: @escaping (_ url: URL?, _ error: Error?) -> Void) {
        let exportSession: AVAssetExportSession = AVAssetExportSession(asset: compostion, presetName: AVAssetExportPresetHighestQuality)!
